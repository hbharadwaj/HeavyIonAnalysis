...
void plot_sys_inverted_pp(TString file_label="",TString in_test_label="Data"){
    if(file_label=="")file_label=label;

    TString file_path = Form("/home/llr/cms/bharikri/Projects/Photon_Analysis/CMSSW_10_3_3_patch1/src/HeavyIonsAnalysis/JetAnalysis/test/Unfolding/OutputDir_pp/Unfold_%s",label.Data());
...
void plot_sys(TString in_file,TString in_test_label){
    ...
    do
    {
        ...
        if(flag_invert){
            Plot_hist_ratio({h_Folded_Raw_X},{"Folded_Raw",var_nam[ivar],"Refolded/Raw","pp_"+dir_cent_string+test_label[test_index]+"_Refolded_MatInvertVs"+var_arr[ivar]+file_string},"bcenter_label_leg",sel);
            if(test_index!=kNoTest)
                Plot_hist_ratio({h_Unfolded_True_X},{"Unfolded_True",var_nam[ivar],"Unfolded/True","pp_"+dir_cent_string+test_label[test_index]+"_Unfolded_MatInvertVs"+var_arr[ivar]+file_string},"bcenter_label_leg",sel);

            Plot_hist({h_True_X,h_Bayesian_Unfolded_X[iter_ref],h_Unfolded_Invert_X},{"True MC","D'Agostini","Matrix Inversion",var_nam[ivar],"1/N_{jet} dN/d"+var_nam[ivar],"pp_"+dir_cent_string+test_label[test_index]+"_True_Unfolded_Matinvert_"+var_arr[ivar]+file_string},"rightlabel",sel);
        }
        else{
        ...
        if(test_index==kBottomline){
            ...
            histname_input.push_back("pp_"+dir_cent_string+test_label[test_index]+"_Chi2_ndf_bottomline_"+var_arr[ivar]+file_string);
            ...
            histname_input.push_back("pp_"+dir_cent_string+test_label[test_index]+"_Graphical_Bottomline_test_"+var_arr[ivar]+file_string);
...
int main(int argc, char* argv[]){
    ...
    // if(argc==7){
        // output_path = argv[1];
        // label = argv[2];
        // plot_sys_inverted_pp(argv[3],argv[4],atoi(argv[5]),atoi(argv[6]));
    // }
